 ...
 *
 * ── ⚠️ 這個數字管的是「讀不讀得出來」，不是「讀出來是什麼意思」（票券 50）─────────
 * `doc_schema_version` 是 **PM node schema 的可讀性版本**，不是**值語意**的版本。兩者的
 * 差別決定了「這次要不要 +1」，而下一個人很容易推錯方向，所以寫在這裡：
 *
 * | | 長相 | 舊 doc 會怎樣 | 歸誰管 |
 * |---|---|---|---|
 * | **可讀性** | 節點型別消失、attr 被移除、`extras` attr 改名、群演從 attr 搬進 node | `fromJSON` **丟例外**、或 attr 被靜默改寫 | 這個模組 —— **+1、補一節遷移** |
 * | **值語意** | 某個 attr 裡裝的**值**換了形狀（票券 44–50 的人數：`count: 3` → `{ kind: "range", from: 3, to: 5 }`） | 照樣 `fromJSON` 得出來，只是讀出一個舊形狀的值 | `sceneExtras`（§6.6 的讀取路徑正規化）—— **不 +1** |
 *
 * 人數那一串票因此**沒有**升版：`extras` 是 `{ default: [] }`，PM 不驗陣列裡面裝什麼，
 * 帶舊欄位的 doc 原封不動讀得出來。值怎麼讀是 `packages/schema` 的 `readCount` 的本分，
 * 而它刻意**不拿版本號**（那條容錯的框定寫在該函式的註解裡）。
 */
