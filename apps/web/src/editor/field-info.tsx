 ...
 *
 * **它只講定義與出路，不勸、不擋。** 四個欄位各自有一件編劇會踩到的事 —— 雜景為什麼長在
 * 內外欄、地點為什麼只能填一個、登場人物的判準是入鏡而不是台詞 —— 而這些話必須在**打字之前**
 * 就在那裡，不是被拒絕之後才冒出來。這是 ADR-0006 那條方法論的同一個形狀：事前標注勝過
 * 事後提示，因為它在錯誤發生前作用。
 *
 * 時間欄沒有坑，但也給一份：四欄三個有 icon、一個沒有，看起來像漏了。
 *
 * ── 無障礙 ────────────────────────────────────────────────────────────
 * icon 本身 `tabIndex={-1}`：編劇每一場都要用 Tab 走一遍 metadata，四個 icon 會讓那條路
