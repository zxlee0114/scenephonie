 ...
 *
 * **共用同一套子選單**（版面第二輪，驗收回饋 2026-09-13）—— 骨架相同，措辭不同：
 *
 * ```
 * ↰ 回上一步，改群演名稱      ← 編輯那側是 `↰ 不修改數量（8），回上一步`
 * ▌                          ← 格子（兩側都在第二列，**焦點與高亮都在這裡**）
 * 不特別指定數量（若干）       ← 編輯那側是 `若干`
 *                             ← 編輯那側這裡還有一列 `1`
 * ✕ 放棄新增群演              ← 編輯那側是 `↩︎ 不修改，返回`
 * ```
 *
 * 1. **新增這一關只有兩條路**：打一個數字，或者不說。所以 `1` 那一列拿掉了（它與格子裡
 *    打一個 `1` 完全重複），而剩下那一列的措辭說的是**選擇本身** —— 他不是挑了「若干」
 *    這個數量，他是決定不說。編輯那一側 `1` 留著：從 8 改成 1 是一個真的捷徑。
 * 2. **預設值靠措辭說，不靠位置**。上一版把高亮放在 `若干` 那一列上（好讓「什麼都不做會
 *    記成什麼」有個看得見的位置），編劇驗收時推翻：游標在格子裡、高亮在別處，一份選單
 *    出現兩個重點。現在高亮只說「下一顆鍵會打在哪」，預設值寫在那一列的字裡。
...
export function ExtrasField({
  ...
  if (countStage) {
    ...
    // `若干` 排在格子底下 —— 兩側同一個位置，但**措辭不同**（驗收回饋 2026-09-13）：
    //
    // | | 那一列印什麼 |
    // |---|---|
    // | 編輯 | `若干` —— 與 `1`、格子並列的三個答案之一 |
    // | 新增 | `不特別指定數量（若干）` —— 它是**這一關的退場**，不是一個數字 |
    //
    // 新增那一側整關只剩「打一個數字」與「不指定」兩條路（`1` 那一列拿掉了，見下），所以
    // 這一列說的是**選擇本身**：他不是挑了「若干」這個數量，他是決定不說。括號裡留著
    // `若干`，因為那才是 chip 上與場次表上會看到的字。
    rows.push({
      key: "some",
      label: heldForCount ? SOME_LABEL : `不特別指定數量（${SOME_LABEL}）`,
      run: () => pickCount({ kind: "some" }),
    });
    // `1` **只在編輯那一側**（驗收回饋 2026-09-13）。那裡它是一個真的捷徑：從 8 改成 1
    // 是常見的一下。新增那一側它與格子裡打一個 `1` 完全重複，夾在「不指定」與「放棄」
    // 中間只是讓三條路看起來像四條 —— 而且**與第一列不重複時才印**那條規矩本來就是為了
    // 不印兩次同一個答案。
    if (heldForCount && (!now || formatCount(now) !== "1")) {
