...
（五條都有自動測試，人工驗收 2026-09-12 全數通過。）

## Comments

### 2026-09-12 — 收票：人工驗收通過

五條全數通過，實作與 code review 之後沒有再改任何一行程式。

驗收時踩到一個與這張票無關的環境問題，記在這裡免得下一個開 worktree 的人再撞一次：
`.gitignore` 擋掉 `.env` 與 `.env.*`，所以 `git worktree add` 不會把 `apps/web/.env`
帶進 worktree，`pnpm dev` 起來完全沒有環境變數。做一個指向主 checkout 那份的 symlink
即可（不要複製 —— 秘密只留一份，主檔改了這邊跟著改）。
