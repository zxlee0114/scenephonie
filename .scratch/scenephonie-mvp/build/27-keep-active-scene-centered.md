...
- [x] 在一份長度超過一個畫面的劇本底部新增下一場，新場次落在畫面中段（非底緣），下方仍有可視餘裕
- [x] 新場次的焦點串接（焦點落在新場次的內外景欄）不回歸
- [x] 短劇本（內容不足一畫面）時不產生詭異的捲動或大片空白
...
**本機驗收（2026-09-04）** —— 四條全過。使用者另外發現一個相鄰的問題：⌘+A 全選到整份劇本
再刪除時，doc 會變成零場次（schema 是 `scene*`，空 doc 合法），畫面上不剩任何可點的東西 ——
另開票券 32。
