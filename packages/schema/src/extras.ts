 ...
 *   —— 值語意的守門人就是這裡。
 * - 它在的理由是**有些路徑繞得過寫入端**：編輯器內的複製貼上、直接灌 PM JSON 的匯入、
 *   手寫的測試 fixture 都生得出一個光禿禿的 `8`，而那個 `8` 有一個**唯一誠實的讀法**。
 *   刪掉它，那些路徑會靜靜掉成「若干」—— 不報錯，只是把編劇寫的數字弄丟。
 *   ⚠️ 走 command 的匯入**不在**這個名單裡：`setSceneExtras` 對裸數字是拒收的（見該處註解），
 *   容錯只覆蓋繞過 command 的那幾條路。
 *
